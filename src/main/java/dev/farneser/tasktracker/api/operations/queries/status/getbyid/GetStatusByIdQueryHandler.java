...
public class GetStatusByIdQueryHandler implements QueryHandler<GetStatusByIdQuery, StatusView> {
    ...
    public StatusView handle(GetStatusByIdQuery query) throws NotFoundException {
        Status column = columnRepository.findByIdAndProjectId(query.getStatusId(), query.getUserId()).orElseThrow(() -> new NotFoundException("Kanban column with id " + query.getStatusId() + " not found"));
