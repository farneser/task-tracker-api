...
public class GetColumnByIdQueryHandler implements QueryHandler<GetColumnByIdQuery, ColumnView> {
    ...
    public ColumnView handle(GetColumnByIdQuery query) throws NotFoundException {
        KanbanColumn column = columnRepository.findByIdAndUserId(query.getColumnId(), query.getUserId()).orElseThrow(() -> new NotFoundException("Kanban column with id " + query.getColumnId() + " not found"));

        log.debug("Column found: {}", column);

        ColumnView view = modelMapper.map(column, ColumnView.class);
